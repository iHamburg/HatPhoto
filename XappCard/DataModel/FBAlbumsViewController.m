...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FBAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];


	if (!cell) {
		cell = [[FBAlbumCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CellIdentifier];
...
//		cell.imageView.image = [[SpriteManager sharedInstance]placeholderImage];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	FBAlbum *album = fbManager.fbAlbums[indexPath.row];
//	if (!photoDisplayVC) {
		photoDisplayVC = [[PhotoDisplayViewController alloc]init];
//	}
	
	photoDisplayVC.fbPhotos = album.fbPhotos;
	photoDisplayVC.source = PhotoSourceFacebook;
